...
function buildRules(conf, ruleList) {
  ...
  if (trigger[conf.tag]) {
    if (conf.required) {
      ...
      let message = Array.isArray(conf.defaultValue) ? `Please select at least one ${conf.vModel}` : conf.placeholder
...
function buildBeforeUpload(conf) {
  ...
  if (conf.accept) {
    ...
    if(!isAccept){
      this.$message.error('Should select ${conf.accept} type files')
    }`
